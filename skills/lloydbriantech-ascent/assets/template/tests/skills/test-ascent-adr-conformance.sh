...
# Reusable: check supersession bidirectionality (matches skill Step 4 logic)
# For each ADR with "Superseded by ADR-NNN", verify the new ADR's Context
# contains the canonical phrase "This ADR supersedes ADR-MMM".
check_supersession_bidirectional() {
  ...
  for adr_file in "$decisions_dir"/ADR-*.md; do
    [ -f "$adr_file" ] || continue
    local new_num
    new_num=$(grep -oE 'Superseded by ADR-[0-9]+' "$adr_file" 2>/dev/null | grep -oE '[0-9]+' | head -1)
    [ -z "$new_num" ] && continue
    local old_adr_num
    old_adr_num=$(basename "$adr_file" | grep -oE 'ADR-[0-9]+' | grep -oE '[0-9]+')
    local new_adr
    new_adr=$(ls "$decisions_dir"/ADR-"$(printf '%03d' "$((10#$new_num))")"*.md 2>/dev/null | head -1)
...
# Clean case: ADR-004 says "Superseded by ADR-008", ADR-008 Context opens with
# "This ADR supersedes ADR-004: SQLite-WAL." → bidirectional, PASS.
...
# Clean ADR-008: has the canonical phrase in Context
